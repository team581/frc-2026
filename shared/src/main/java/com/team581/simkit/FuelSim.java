...
public class FuelSim {
  ...
  protected static final double DRAG_COF = 0.465; // dimensionless
  ...
  protected boolean spawnedStartingFuel = false;

  protected int maxAdditions = Integer.MAX_VALUE;
  ...
  protected double shooterHeight;
  protected double shooterForwardOffset;
  protected double shooterWidth;
  ...
  public void launchFuel(
    ...
    Pose3d launchPose =
        new Pose3d(this.robotPoseSupplier.get())
            .plus(
                new Transform3d(
                    new Translation3d(
                        Distance.ofBaseUnits(shooterForwardOffset, Meters),
                        Distance.ofBaseUnits((shooterWidth / 2) * (1 - Math.random() * 2), Meters),
                        launchHeight),
                    Rotation3d.kZero));
    ...
    double xVel =
        horizontalVel * Math.cos(shotYaw.plus(launchPose.getRotation().getMeasureZ()).in(Radians));
    double yVel =
        horizontalVel * Math.sin(shotYaw.plus(launchPose.getRotation().getMeasureZ()).in(Radians));
  ...
  public void registerRobot(
    ...
    this.shooterHeight = launchHeight.in(Meters);
    this.shooterForwardOffset = shooterForwardOffset.in(Meters);
    this.shooterWidth = shooterWidth.in(Meters);
  ...
  public void registerRobot(
    ...
    this.shooterHeight = launchHeight;
    this.shooterForwardOffset = shooterForwardOffset;
  ...
  /**
   * Sets the maximum number of fuels allowed on the field
   *
   * @param maxSize maximum number of fuels
   */
  public void setMaxAdditions(int maxSize) {
    this.maxAdditions = maxSize;
  }
  ...
  public void spawnFuel(Translation3d pos, Translation3d vel) {
    if (fuels.size() >= (spawnedStartingFuel ? 408 : 0) + maxAdditions) {
      for (int i = 0; i < fuels.size(); i++) {
        Fuel f = fuels.get(i);
        if (f.pos.getZ() < FUEL_RADIUS + 0.05) {
          fuels.remove(i);
          break;
        }
      }
