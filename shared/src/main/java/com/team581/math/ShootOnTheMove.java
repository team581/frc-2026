...
public class ShootOnTheMove {
  ...
  public Translation2d getRadialVelocityCompensatedGoal(Translation2d robot, Translation2d target, ChassisSpeeds robotVelocity) {
    var result = target;
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        ...
        double radialVelocityMag = (robotVelocity.vxMetersPerSecond * angleToTarget.getCos()) +
        ...
        result = target.minus(offset);
            ...
            DogLog.log("ShootOnTheMove/TargetRadial", new Pose2d(target.getX(), target.getY(),target.getAngle()));
    return result;

}
public Translation2d getTangentialVelocityCompensatedGoal(Translation2d robot, Translation2d target, ChassisSpeeds robotVelocity) {
    var result = target;
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        ...
        double tangentialVelocityMag = (robotVelocity.vxMetersPerSecond * tangentialDir.getX()) +
        ...
        result = target.minus(tangentialDir.times(tangentialVelocityMag * tof));
                ...
                DogLog.log("ShootOnTheMove/TargetTangential", new Pose2d(target.getX(), target.getY(),target.getAngle()));
